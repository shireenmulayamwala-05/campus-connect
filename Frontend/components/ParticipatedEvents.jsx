    ...
    const fetchEvents = async () => {
      try {
        ...
        const { data } = await axios.get(
          "http://localhost:8000/api/v2/events/get-participatedEvent",
          { withCredentials: true }
        );

        if (data.success) {
          setEvents(data.participatedEvents || []);
        } else {
          setError(data.msg || "Failed to fetch participated events");
          toast.error(data.msg || "Failed to fetch participated events");
        }
      } catch (err) {
        const errorMsg =
          err.response?.data?.msg || "Error fetching participated events";
        setError(errorMsg);
